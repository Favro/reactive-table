    ...
    if (filter) {
      if (_.isObject(filter)) {
        if (filter.$nor) {
          ...
          for (var subFilter of filter.$nor) {
            fieldQueries = fieldQueries.concat(_.map(filterFields[index], function (field) {
              var query = {};
              query[field] = subFilter;
              return query;
            }));
          }
          if (fieldQueries.length) {
              queryList.push({'$nor': fieldQueries});
          }
        } else {
