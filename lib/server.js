...
ReactiveTable.getFilterQuery = getFilterQuery;
...
ReactiveTable.publish = function (name, collectionOrFunction, selectorOrFunction, settings) {
    Meteor.publish("reactive-table-" + name, function (publicationId, filters, fields, options, rowsPerPage) {
      ...
      var fullCursor;
      var count;
      if (settings && settings.enablePageCountReactivity) {
        fullCursor = collection.find(filterQuery);
        count = fullCursor.count();
      } else {
        count = collection.find(filterQuery).count();
      }
